...
async fn example_transfer_instruction() -> Result<()> {
    ...
    let client = RpcClient::new_with_commitment(RPC_URL_LOCAL.to_string(), CommitmentConfig::confirmed());
    ...
    let airdrop_signature = client.request_airdrop(&sender.pubkey(), LAMPORTS_PER_SOL).await?;
